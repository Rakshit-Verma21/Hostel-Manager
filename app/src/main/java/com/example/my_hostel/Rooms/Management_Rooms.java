...
import com.example.my_hostel.Adapters.Rooms.Rooms_display_Adapter;
...
public class Management_Rooms extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        EditText seatchEditText = searchView_filter_rooms.findViewById(androidx.appcompat.R.id.search_src_text);
        seatchEditText.setTextColor(getResources().getColor(R.color.black));
        seatchEditText.setHintTextColor(getResources().getColor(R.color.black));
        getRooms();
        recyclerView_rooms.setLayoutManager(new GridLayoutManager(this,3));
        ...
        for(String uid : room_uid_list)
        {
            database.child(ROOM_PATH).child(uid).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot)
                {
                    if(snapshot.exists())
                    {
                        if(query.toUpperCase().contains(uid))
                        {
                        ...
                        if(String.valueOf(price).contains(query))
                        {
                            if(!filtered_list.contains(uid))
                            {
                                filtered_list.add(uid);
